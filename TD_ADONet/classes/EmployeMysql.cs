...
namespace EmployeDatas.Mysql
{
    class EmployeMysql
    {
        ...
        public int AugmenterSalaireCurseur()
        {
            ...
            MySqlTransaction mySqlTransaction = this.connexion.BeginTransaction();
            try
            {
                cmd = new MySqlCommand(requete, connexion);
                reader = cmd.ExecuteReader();
                int nb = 0;
                string sqlUpdate = @"UPDATE employe SET employe.salaire = employe.salaire * 1.03 WHERE employe.numemp = @numemp";
                MySqlCommand mySqlCommandUpdate = new MySqlCommand(sqlUpdate, connexion);

                while (reader.Read())
                {
                    mySqlCommandUpdate.Parameters[0].Value = reader.GetValue(0);
            ...
            catch (MySqlException ex)
            {
                ...
                throw new Exception("Erreur à la fonction AugmenterSalaireCurseur");
